...
from router_order import load_routers, router_slug
...
def iter_router_names(cfg: dict[str, object]) -> list[str]:
    return [router.name for router in load_routers(cfg)]


def iter_exit_hubs(cfg: dict[str, object]) -> list[ExitHub]:
    return build_config_data(cfg).exit_hubs


def iter_exit_names(cfg: dict[str, object]) -> list[str]:
    return [hub.name for hub in iter_exit_hubs(cfg)]


def process_routers(cfg: dict[str, object], key_dir: Path) -> None:
    for router_name in iter_router_names(cfg):
        slug = router_slug(router_name)
...
def build_ssh_config(cfg: dict[str, object], key_dir: Path) -> str:
    ...
    for router in load_routers(cfg):
        slug = router.slug
    ...
    for exit_hub in iter_exit_hubs(cfg):
...
def write_ssh_config(cfg: dict[str, object], key_dir: Path) -> None:
    ssh_config_path = deploy_ssh_config_path_from_config(cfg)
    text = build_ssh_config(cfg, key_dir)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Generate per-router/per-server SSH keys and write authorized_keys "
            "plus deploy SSH config"
        )
    )
    ap.add_argument(
        "--config",
    ...
    process_routers(cfg, key_dir)
    process_servers(cfg, key_dir)
    write_ssh_config(cfg, key_dir)
