...
`routers/example` и `servers/example` являются шаблонами. Конкретные router-директории создаются как lowercase slug рядом с `routers/example`, а server-директории сохраняют исходное имя exit-а.
...
В этом примере LAN `Branch01` может обращаться к самому роутеру `Core01` и форвардиться в LAN `Core01` и `Lab01`.
...
{
  "name": "Core01",
  "packages": ["+luci-proto-wireguard", "+openvpn-openssl", "-tcpdump"]
}
```
